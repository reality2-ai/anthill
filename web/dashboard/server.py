#!/usr/bin/env python3
"""Tiny HTTP server for the Spec Confidence dashboard with inline APIs."""
...
class DashboardHandler(SimpleHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, directory=os.path.join(REPO_ROOT, 'web'), **kwargs)
...
def main():
    parser = argparse.ArgumentParser(description='Serve dashboard with inline API')
    parser.add_argument('--host', default='0.0.0.0')
    parser.add_argument('--port', type=int, default=8080)
    args = parser.parse_args()

    server = ThreadingHTTPServer((args.host, args.port), DashboardHandler)
    print(f"Dashboard server running at http://{args.host}:{args.port}/dashboard/")
    print(f"  Serving from: {os.path.join(REPO_ROOT, 'web')}")
    try:
