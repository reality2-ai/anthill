...
# Map old column IDs to refutation-framed labels
COLUMNS: List[Dict[str, Any]] = [
    ...
    {'id': 'test_spec', 'label': 'Survived Test Vectors', 'icon': '⚔️',
     ...
     'conjecture': 'The spec\'s claims can be expressed as concrete, deterministic test cases — and those tests pass. If the spec says "X produces Y", there exist vectors proving it. Adversarial vectors specifically designed to break edge cases have been tried and failed.',
    ...
    {'id': 'built', 'label': 'Survived Implementation', 'icon': '🔨',
     ...
     'labels': ['Partial/WIP', 'Complete', 'Optimised']},
    {'id': 'testing', 'label': 'Survived Automated Refutation', 'icon': '🧪',
     'refutation_type': 'dual_model_audit',
     'conjecture': 'The implementation survives automated attempts to break it — fuzz testing, property-based testing, model checking, and adversarial AI review. Machine-generated attacks at scale have not found violations of spec guarantees.',
     'signals': ['unit', 'automated', 'roundtrip', 'human'],
     'labels': ['Unit tests', 'Automated vectors', 'Roundtrip/fuzz', 'Human validation']},
    {'id': 'integrated', 'label': 'Survived Integration Stress', 'icon': '🔗',
     'refutation_type': 'integration_test',
     'conjecture': 'The spec works correctly when composed with other specs in a real system. Assumptions made by one spec about another have been tested at the boundaries. Cross-spec interactions, ordering dependencies, and emergent behaviours have been stressed without failure.',
     'signals': ['component', 'cross_component', 'system'],
     'labels': ['Component tested', 'Cross-component', 'System-level']},
    {'id': 'field', 'label': 'Survived Field Deployment', 'icon': '🌍',
     'refutation_type': 'field_deployment',
     'conjecture': 'The spec survives contact with the real world — actual hardware, actual weather, actual users, actual failure modes. Lab conditions did not hide problems that the field reveals. This is the most expensive and most convincing form of refutation.',
     'signals': ['lab', 'pilot', 'production'],
     'labels': ['Lab/bench', 'Pilot deployed', 'Production proven']},
]
...
DEFAULT_ROSTER = {
    '_defaults': {
        ...
        'MK': {'review1': 'Roy Davies', 'review2': 'Mariko Ops', 'expert': 'Council auditor'},
        'TH': {'review1': 'Roy Davies', 'review2': 'Thurisaz Ops', 'expert': 'External auditor'},
    }
}
...
def _compute_refutation_breadth(reviews_list: List[Dict[str, Any]], current_commit: Optional[str]) -> Dict[str, Any]:
    ...
    stale_count = len(survived_types_stale - survived_types)  # Only count stale if not also fresh
...
def _normalize_review(review: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'outcome' not in normalized:
        # Legacy reviews with status 'approved' → survived
...
def build_state() -> Dict[str, Any]:
    ...
    for cat in ['R2 Core', 'Mariko', 'Thurisaz', 'Other']:
        if cat not in categories:
            continue
        cat_codes = categories[cat]
        # Separate standalone specs (no deps AND nothing depends on them within this category)
        standalone = [c for c in cat_codes
                      if not deps.get(c, {}).get('depends_on', [])
                      and not deps.get(c, {}).get('depended_by', [])]
        connected = [c for c in cat_codes if c not in standalone]
        topo = topo_sort_category(connected, deps)
        ...
        if standalone:
            ordered_cats.append((f'{cat} — Standalone', sorted(standalone)))
    ...
    for code in deps:
        ...
        if has_any_reviews and current_commit:
            # Check if ALL reviews are stale
...
def _compute_evidence_staleness(
    ...
    for col_id in ['reviewed', 'test_spec', 'built', 'testing', 'integrated', 'field']:
        ...
        if not has_evidence:
            ...
            continue
        # If we have evidence but no current commit, can't determine staleness
        if not current_commit:
            staleness[col_id] = False
            continue
        # Check if any review with a matching commit exists
        has_fresh = any(
