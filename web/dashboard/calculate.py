...
Every cell in the 2D table (spec × pipeline stage) gets a continuous
...
# 0.15 means if all deps are at 0%, the dependent loses 15% of its score.
# Kept moderate to flag dependency issues without cascading zeroes.
DEP_PROPAGATION_WEIGHT = 0.15

# Dependency confidence below this threshold triggers an inherited-drop warning.
DEP_DROP_THRESHOLD = 0.60

# Column definitions: each column has sub-evidence with weights that sum to 1.0
COLUMNS = {
    'reviewed': {
        'weight': 0.15,  # column weight in overall score
        'signals': {
            'review1': 0.40,
            'review2': 0.30,
            'expert': 0.30,
...
def main():
    ...
    for code, info in deps.items():
        ...
        # Each changed dep contributes a small penalty. Total is capped to
        # prevent cascading zeroes when multiple foundation specs update.
        dep_version_changes = []
            ...
            if dep_code in deps and dep_code in reviews:
                ...
                if approved and dep_approved:
                    ...
                    if dep_ver and latest_dep_review_ver and dep_ver != latest_dep_review_ver:
                        dep_version_changes.append(dep_code)
        if dep_version_changes:
            # Diminishing penalty: first dep -0.05, subsequent -0.02 each, cap at -0.12
            dep_penalty = min(0.12, 0.05 + max(0, len(dep_version_changes) - 1) * 0.02)
            overall -= dep_penalty
            # Mild cell reduction (flag for revalidation, not destruction)
            for col in ['testing', 'integrated', 'field']:
                cells[col] = max(0.0, cells[col] - 0.05)
            dep_names = ', '.join(dep_version_changes)
            penalties.append(f"Dependency version changed: {dep_names} (-{dep_penalty:.2f})")
