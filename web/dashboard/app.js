(function () {
  ...
  const SPEC_REPO_BASE = 'https://github.com/reality2-ai/anthill/blob/elixir-rebuild/';
  const CODE_REPO_BASE = 'https://github.com/reality2-ai/anthill/tree/elixir-rebuild/';
  ...
  let sortMode = localStorage.getItem('r2-dash-sort') || 'topo'; // 'topo' or 'alpha'
  ...
  function initSortToggle() {
    ...
    if (!toggle) return;
    // Store topo indices immediately
    indexTopoOrder();
    // Apply saved preference
    updateToggleLabel(toggle);
    if (sortMode === 'alpha') applySortOrder();
    toggle.addEventListener('click', () => {
      sortMode = sortMode === 'topo' ? 'alpha' : 'topo';
      localStorage.setItem('r2-dash-sort', sortMode);
      updateToggleLabel(toggle);
      applySortOrder();
    });
  }
  ...
  function updateToggleLabel(toggle) {
    toggle.textContent = sortMode === 'alpha' ? 'Spec A→Z' : 'Spec ⇅';
    toggle.title = sortMode === 'alpha' ? 'Sorted alphabetically (click for dependency order)' : 'Sorted by dependency (click for alphabetical)';
  }

  function indexTopoOrder() {
    const tbody = document.querySelector('.grid-table tbody');
    if (!tbody || tbody.dataset.topoIndexed) return;
    let idx = 0;
    tbody.querySelectorAll('tr.spec-row').forEach(tr => {
      tr.dataset.topoIdx = idx++;
    });
    tbody.dataset.topoIndexed = 'true';
  }
  ...
  function applySortOrder() {
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
      if (tr.classList.contains('cat-row')) {
        current = { catRow: tr, specRows: [] };
        groups.push(current);
      } else if (tr.classList.contains('spec-row') && current) {
        current.specRows.push(tr);
      }
    });
    // Sort spec rows within each group independently
    groups.forEach(g => {
      if (sortMode === 'alpha') {
        g.specRows.sort((a, b) => (a.dataset.spec || '').localeCompare(b.dataset.spec || ''));
      } else {
        g.specRows.sort((a, b) => parseInt(a.dataset.topoIdx || '0', 10) - parseInt(b.dataset.topoIdx || '0', 10));
      }
    });
    // Re-append in order
    groups.forEach(g => {
      tbody.appendChild(g.catRow);
      g.specRows.forEach(tr => tbody.appendChild(tr));
    });
  }
  ...
  // ── Detail panel (where complexity lives) ─────────────────────────────
  ...
  function showDetail(code, focusCol) {
    ...
    const specLink = sd.path ? SPEC_REPO_BASE + encodeURIComponent(sd.path) : null;
    ...
    if (specLink) {
      html += '<a class="repo-link" href="' + specLink + '" target="_blank" rel="noopener">Spec</a>';
    }
    html += '<a class="repo-link" href="' + CODE_REPO_BASE + '" target="_blank" rel="noopener">Code</a>';
    html += '<button class="close-btn" onclick="closeDetail()">Close</button></div></div>';

    // Staleness timeline — ambient, not alarming
    ...
    if (inheritedDrops.length) {
      html += '<section><div class="inherited-drop-alert"><strong>⚠ Inherited confidence drop</strong>';
    ...
    // GitHub Issues (linked from project board)
    const ghIssues = window.__GITHUB_ISSUES__?.[code];
    if (ghIssues?.length) {
      html += '<section><div class="section-title">GitHub Issues</div><div class="github-issues">';
      // Group by type
      const epics = ghIssues.filter(i => i.type === 'epic');
      const sprints = ghIssues.filter(i => i.type === 'sprint');
      const others = ghIssues.filter(i => !i.type);

      const renderIssue = (iss, indent) => {
        const prefix = indent ? 'style="padding-left:1rem"' : '';
        const badge = iss.type === 'epic' ? '<span class="badge badge-type">epic</span> ' :
                      iss.type === 'sprint' ? '<span class="badge badge-type">sprint</span> ' : '';
        const url = 'https://github.com/reality2-ai/' + iss.repo + '/issues/' + iss.number;
        return '<div class="gh-issue" ' + prefix + '>' + badge +
          '<a href="' + url + '" target="_blank" rel="noopener" class="repo-link">' +
          iss.repo + '#' + iss.number + '</a> ' + escHtml(iss.title) + '</div>';
      };

      epics.forEach(epic => {
        html += renderIssue(epic, false);
        // Show child sprints indented under their epic
        sprints.forEach(s => { html += renderIssue(s, true); });
      });
      if (!epics.length) {
        sprints.forEach(s => { html += renderIssue(s, false); });
      }
      others.forEach(o => { html += renderIssue(o, false); });
      html += '</div></section>';
    }
  ...
  function renderDepGraph(sd) {
    ...
    node(centerX, centerY, sd.name.split(':')[0], 'var(--accent)');
  }

  // ── Grid updates (calm — only dot size + opacity) ─────────────────────
  ...
  // ── Summary (includes freshness metric) ───────────────────────────────
  ...
  async function onEvChange(cb) {
    ...
    if (isGitHubPages()) {
      // On Pages, show CLI command since evidence changes are lightweight
      updateCmdPanel();
      return;
    }
  ...
  async function saveChange(spec, key, value) {
    ...
    } catch { apiAvailable = false; showToast('API unavailable — run dashboard/server.py', true); return false; }
  }
  ...
  function updateCmdPanel() {
    ...
    Object.entries(pendingChanges).forEach(([spec, items]) => {
      Object.entries(items).forEach(([key, val]) => cmds.push(`python3 dashboard/review.py ${spec} --set ${key}=${val}`));
    });
  ...
  const GITHUB_REPO = 'reality2-ai/anthill';

  function isGitHubPages() {
    return window.location.hostname.includes('github.io');
  }

  function openReviewAsIssue(params) {
    const outcome = params.outcome === 'survived' ? '✅ Survived' : '💥 Falsified';
    const title = `Review: ${params.spec} — ${params.outcome} (${params.type}/${params.severity})`;
    const body = [
      '## Review Submission',
      '',
      '| Field | Value |',
      ...
      '',
      params.note ? `### Notes\n${params.note}` : '',
      params.falsification ? `### Falsification\n${params.falsification}` : '',
      '',
      '---',
      '*Submitted from the R2 Specification Dashboard*',
    ].filter(Boolean).join('\n');

    const url = `https://github.com/${GITHUB_REPO}/issues/new?`
      + `title=${encodeURIComponent(title)}`
      + `&body=${encodeURIComponent(body)}`
      + `&labels=${encodeURIComponent('review')}`;
  ...
  async function submitReviewFromDialog() {
    ...
    if (isGitHubPages()) {
      openReviewAsIssue({
        ...
        outcome,
        note: note || '',
        falsification: outcome === 'falsified' ? note : '',
      });
      closeReviewDialog();
      return;
    }

    // Local API fallback (Pi server)
  ...
  // ---- Conjecture popup (hover on spec name) ----
  ...
  if (popup) {
    ...
    // Column header hover — show overarching conjecture for that refutation level
