...
impl StorageBackend for CborGitBackend {
    fn load_graph(&self, name: &str) -> StoreResult<Option<GraphData>> {
        ...
        // Try CBOR first — but fall through to JSON if CBOR lacks citations that JSON has.
        if cbor_path.exists() {
            ...
            let cbor_has_cites = data.edges.iter().any(|(_, _, e)| !e.citations.is_empty());

            // If CBOR has citations (or JSON doesn't exist), use CBOR.
            if cbor_has_cites || !json_path.exists() {
                return Ok(Some(data));
            }

            // JSON exists and CBOR lacks citations — check if JSON has them.
            if let Ok(contents) = std::fs::read_to_string(&json_path) {
                if let Ok(json_data) = serde_json::from_str::<GraphData>(&contents) {
                    let json_has_cites = json_data.edges.iter().any(|(_, _, e)| !e.citations.is_empty());
                    if json_has_cites {
                        log::info!("Loading '{}' from JSON (has citations that CBOR lacks)", name);
                        // Save JSON data as CBOR so future loads use the complete version.
                        if let Err(e) = self.save_graph(name, &json_data) {
                            log::warn!("Failed to persist JSON→CBOR migration for '{}': {}", name, e);
                        }
